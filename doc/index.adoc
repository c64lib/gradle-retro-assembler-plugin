...
image::img/ci-flow.png[]

With Build Tool it is possible to generate executable artifacts and deploy them on demand i.e. as part of GitHub releases.
...
[NOTE]
====
Both `resolveDevDeps` and `downloadDeps` tasks are *ALWAYS* downloading the resources from the net.
This means that if you are offline the build will fail.
This is a naive approach because these resources are saved on disk and can be potentially reused.
There are plans to improve it in the future.
====

It is possible to exclude any task from execution with following syntax:

[source,bash]
----
gradlew build -x resolveDevDeps -x downloadDeps
----

This was you can avoid redundant downloading or still work while offline.

Similarly, you can exclude running unit tests:
...
The build tool allows to specify both the directories as well as which files to use.
...
It has been observed that Vice 3.4 and newer are not working properly with Retro Assembler tool.
If you encounter any problems, please downgrade your Vice to 3.3.
Appropriate bug issue has been created https://github.com/c64lib/gradle-retro-assembler-plugin/issues/49[here].
...
The reason for this option is that in 3.0-3.9 range there were two CTM8 versions supported: CTM8 and CTM8.2. The version CTM8 becomes quickly obsolete and was retained just for backward compatibility reasons.
