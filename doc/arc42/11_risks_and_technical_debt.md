...
|---|------|----------|--------|---------------------|
| D1 | **`crunchers` domain missing from the knowledge base** ✅ **RESOLVED** | `doc/kb/domain.md` previously omitted `crunchers` (`crunchers/exomizer` exists in code with `CrunchMemUseCase`/`CrunchRawUseCase`) — now `doc/kb/domain.md` documents `crunchers` and its `exomizer` subdomain, with a cross-reference to this arc42 §5 | Was: agents/contributors reading only the kb could miss an entire bounded context. Now: kb covers all seven domains | Resolved via [PLAN-0009](../../plans/PLAN-0009_document-crunchers-domain-in-kb.md) issue #156 |
